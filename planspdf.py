...
class PlansPDF:
    ...
    def output_csv(page_dict_parsed, output_file_name):
        ...
        for page_number in sorted(page_dict_parsed):    
            for step_number in sorted(page_dict_parsed[page_number]):
                ...
                # Contracts the list of textboxes to one str with two new lines b/w each box
    ...
    def parse(self, page_range=None):
        ...
        # This is a nested list consisting of a list of str objects for each parsed page
        parsed_pages = []
        ...
        for page_num, text_list in sorted(page_dict_raw.items()):
            ...
            # The last step is stored as a tuple of a step number and list of text
            last_step = None
            ...
            if (not page_num.endswith('-01') or not page_num.endswith('-1')):    
                for text in text_list:
                    ...
                    if step_search:
                        ...
                        step_text = text.replace(step_search.group(), '', 1)
                      
                        # If there was a previous step, save it and make a new one
                        if last_step:
        ...
        if PlansPDF.debug:
            for page_number in sorted(page_dict_parsed):
                ...
                for step_number in sorted(page_dict_parsed[page_number]):
                    ...
                    print("    Text: %s" % str(page_dict_parsed[page_number][step_number]))
